...
// Close popups by Jones altered by Bram
...
async function startBotPlayMatch(page, myCards, quest, claimQuestReward, prioritizeQuest, useAPI) {
	...
	const ratingThresholdMin = process.env.RATING_MIN;
	const ratingThresholdMax = process.env.RATING_MAX;
    ...
    if(ratingThresholdMin!==0&&ratingThresholdMax!==0){
		...
		const userrating = await fetch("https://api.splinterlands.io/players/details?name="+process.env.ACCUSERNAME, {"credentials":"omit","headers":{"accept":"application/json, text/javascript, */*; q=0.01","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"},"referrer":"https://splinterlands.io/?p=battle_history","referrerPolicy":"no-referrer-when-downgrade","body":null,"method":"GET","mode":"cors"})
			.then(response=>response.json())
			.then((response) => {
				let rating = parseInt(response.rating);
				console.log('Current Rating is ' + rating);
				if (rating < ratingThresholdMin || (ratingThresholdMax !== 0 && rating > ratingThresholdMax)){
					console.log('Rating has hit the threshold (min: ' + ratingThresholdMin + ', max: ' + ratingThresholdMax + ')');
					return false;
				}
				return true;
			})
